...
export async function onRequestGet(context) {
  ...
  const categorySelectOptions = Object.entries(CATEGORIES).map(
    ([v, l]) => ({ value: v, label: l })
  );
    ...
    clear() {
      ...
      ${rows.length === 0
        ...
        : html`
                ...
                ${rowData.map(r => html`
                  <tr data-row-id="${escape(r.id)}"
                    ...
                    <td class="col-category_label" data-col="category_label">
                      ${ieSelect('category', r.category, categorySelectOptions)}
                    </td>
          ...
          <script>${raw(listInlineEditScript('/documents/resources/:id/patch', {
            fieldAttrMap: { category: 'category_label' },
          }))}</script>
        `}
    </section>
  `;
