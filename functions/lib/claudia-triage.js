...
function buildSystemPrompt(displayName, today, user, memoryRows) {
  ...
  return [
    ...
    '',
    `YOU HAVE TOOLS — but the enrichment payload at the bottom of your user message ALREADY CONTAINS most of what you need. Consult enrichment FIRST. Tools cost a model round-trip each; reach for them only when enrichment is genuinely insufficient.`,
    '',
    'WHAT IS ALREADY IN THE ENRICHMENT PAYLOAD (do NOT re-fetch via tools):',
    '- enrichment.principal — the full source row (for a doc event: full_text, subject, sender, parent_id, all metadata). Read this first.',
    '- enrichment.related.docs — thread siblings (other emails on the same subject, sorted email_date ASC). The full conversation history is here.',
    '- enrichment.related.opportunities — opps named by number in the source, with recent stage / value / dates.',
    '- enrichment.related.accounts — accounts matching the sender\'s domain.',
    '- enrichment.related.contacts — contacts matching sender_email or named in the body.',
    '- enrichment.related.activities — recent activities tied to the same opp/account.',
    ...
    '- The source mentions an account/contact name that does NOT appear anywhere in enrichment.related — search_accounts to verify it\'s a new vs. existing customer.',
    '- The source mentions an opp number/title that didn\'t match enrichment.related.opportunities — query_db to locate it.',
    '- You need history beyond enrichment.related.audit_recent\'s 5-row cap — query_db for older activities on the entity.',
    '- The principal\'s full_text was truncated (rare — DOC_TEXT_CAP is 150 KB) — read_document to fetch the rest.',
    '- You\'re about to fire an auto-tier write (set_document_category, set_document_retention, refresh_brief) — verify with one tool call beforehand.',
    '',
    'IF YOU CAN ANSWER FROM ENRICHMENT, DO. Skip the tool call. The enrichment was built specifically to make tool calls unnecessary for the common case. A clean event-with-no-tool-calls run reads from cache and decides in one round-trip.',
    '',
...
export async function extractActions(env, { event, enrichment, displayName, today, user, memoryRows, sessionHistory }) {
  ...
  try {
    result = await messagesWithTools(env, {
      ...
      cacheSystem: true,
      cacheTools: true,         // 5-10k tokens of tool defs, stable per invocation
      maxTokens: 4096,
      temperature: 0.2,
      maxToolHops: 6,
    });
