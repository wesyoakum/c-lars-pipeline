...
export async function onRequestPost(context) {
  ...
  const system = [
    ...
    '- Cite ONLY what is in the activity data below. Do NOT introduce facts (opp numbers, sender names, dates, seq numbers, calendar event times) that are not in the data.',
    ...
    `- Do NOT fabricate CALENDAR EVENT times. The upcoming_calendar block below is the ONLY source for events with times. If ${display} has conversationally mentioned an event ("Courtney is graduating", "Stacy's birthday"), you may NOT pair it with a time unless that exact event appears in upcoming_calendar with that time. The name being real does not make a time real. If you want to mention a known-but-uncalendared event, say "not on the calendar" — never assign a guessed time.`,
